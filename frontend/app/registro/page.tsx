...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    // --- NUEVO: Control de Velocidad con la Rueda del Mouse (Scroll) ---
    const onWheel = (e: WheelEvent) => {
      // e.deltaY negativo = scroll hacia adelante (acelerar)
      // e.deltaY positivo = scroll hacia atrás (ralentizar)
      const scrollSensibility = 0.0015;
      params.speed -= e.deltaY * scrollSensibility;

      // Limitar la velocidad para que no retroceda (min) ni se descontrole (max)
      params.speed = Math.max(0.02, Math.min(params.speed, 5.0));
      
      // Si warpDrive está activo, desactivarlo para que el scroll manual tome prioridad
    ...
    window.addEventListener('wheel', onWheel); // Escuchamos el scroll
    ...
    return () => {
      ...
      window.removeEventListener('wheel', onWheel); // Removemos el listener del scroll
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="bg-slate-900/90 backdrop-blur-md p-8 md:p-10 rounded-3xl border border-slate-800 shadow-2xl w-full max-w-md relative z-10">
          ...
          <div className="group">
            ...
            <div className="relative">
               ...
               <input 
                 ...
                 className="w-full bg-slate-950/80 border-b-2 border-slate-800 rounded-t-xl pl-12 pr-4 py-4 text-white outline-none focus:border-orange-500 focus:bg-slate-950 transition-all placeholder:text-slate-700"
